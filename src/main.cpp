...
#if defined(NDEBUG)
# error "PWSCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe7365a0a34829d78a33eda48b3aa497bde07ce670a183bf277411e28ecd47361");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PWSCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PWSCoin Signed Message:\n";
...
{
    // PWSCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10k blocks
    nSubsidy >>= (nHeight / 10000); // PWSCoin: 10k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // PWSCoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // PWSCoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PWSCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PWSCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf60a90b4667149edb262c8a1f7506864076753dc3d7399b6b477b08309faff56");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "06/03/2018 12:00 PWS inleverdatum";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c48c7b4d58df227336f68ba911f0f0cd84b90d8059602090e6b6237bac9423c02b61716446105d2e1a836b51d4cf61300dc5d7a7ca2623a98f588f759fd0afcb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520131250;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084723132;

        if (fTestNet)
        {
            block.nTime    = 1520131173;
            block.nNonce   = 386509634;
        }
if (false && block.GetHash() != hashGenesisBlock)
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x820c667fb032722789a51ef54c2b827c689638d1519bc46a69b08131bacb349f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // PWSCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PWSCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PWSCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PWSCoinMiner(CWallet *pwallet)
{
    printf("PWSCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pwscoin-miner");
    ...
    try { loop {
        ...
        printf("Running PWSCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PWSCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PWSCoinMiner, pwallet));
}
